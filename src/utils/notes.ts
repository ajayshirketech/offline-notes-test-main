...
export function createNote(noteTitle: string, noteContent: string, tags: string[]) {
  const note: Note = {
    title: noteTitle,
    content: noteContent,
    tags: tags,
    ...
    if (note !== undefined) {
      if (note._id === undefined) {
        ...
        if (navigator.onLine) {
          ...
          try {
            await axios.put(`/api/edit-note?id=${note._id}`, { title: updatedTitle, content: updatedContent, tags: updatedTags });
            note.title = updatedTitle;
            note.content = updatedContent;
            note.tags = updatedTags;
    ...
    try {
      ...
      for (const localNote of localNotes) {
        if (localNote.localDeleteSynced === false) {
          ...
          console.log(matchingServerNote)
        ...
        if (localNote.localEditSynced === false) {
          const matchingServerNote = serverNotes.find((serverNote: Note) => localNote._id === serverNote._id);
          if (matchingServerNote !== undefined) {
            if (localNote.lastSyncedTitle !== matchingServerNote.title) {
              console.log("conflict detected");
              // localNote.conflict = true;
              // await editOfflineNote(localNote);
              // await axios.put(`/api/edit-note?id=${localNote._id}`, { conflict: true });
            }
          }
        }
      }
